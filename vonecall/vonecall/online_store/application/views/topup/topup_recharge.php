...
$(document).ready(function(){
 	...
 	$('#submitButton').click(function(e) {
	  ...
	  var senderPhone = ($('input[name=senderPhone]').val());
	    if((countryCode && phoneNumber && amount && senderPhone) !=''){
    ...
    $("#topup_form").validate({
    	rules: {
			phoneNumber:{
				required: true,
				minlength:<?php echo $results->vLocalPhoneNumberLength?>,
				//maxlength:<?php echo $results->vLocalPhoneNumberLength?>,
				number: true
			},
			<?php if(isset($rechargeAmount)){?>				
			amount:{
				required: true,
				range: [ <?php echo is_float($results->vproductMinAmount)?round($results->vproductMinAmount, 2):$results->vproductMinAmount?>, <?php echo is_float($results->vproductmaxAmount)?round($results->vproductmaxAmount, 2):$results->vproductmaxAmount?>]
			},
			<?php }else{?>
			amount:{
				required: true,
  ...
  <div class="center_page_afterlogin">
    ...
    <div class="col_big">
        <div class="box_makepayment ">
	        ...
	        <div class="countryBox">
	        	<div class="flag">
	            	 <div class="countryFlag">
	            	 	<img width="150" height="90" alt="Airtel Bharti India" src="<?php echo $this->config->item('base_url')?>systemadmin/public/uploads/product_logo/<?php echo $results->logoName;?>">
            ...
            <?php echo form_open_multipart(site_url('topup-recharge/'.$results->ppnProductID), array('id'=>'topup_form', 'name'=>'topup_form'));?>
            <label class="label1"> Mobile Number:</label>
            <span style="float: left;">+<?php echo $results->CountryCode?></span> 
            <input type="text" name="phoneNumber" value="<?php echo isset($phoneNumber)?$phoneNumber:''?>" maxlength="<?php echo $results->vLocalPhoneNumberLength?>">
