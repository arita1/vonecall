...
$(document).ready(function(){
 	...
 	$('#submitButton').click(function(e) {
	  ...
	  <?php if(isset($rechargeAmount)){?>	
	   ...
	   <?php }?>
	   if( amount !=''){
			if(confirm('You are about to purchase PIN with amount $'+amount)){
				$('.loading').show();	
				$("#topup_form").submit(); 
				return true;
			}else{
    ...
    $("#topup_form").validate({
		...
		messages: {
			...
			recipientPhone: {
				required: "This field can not be empty",
				minlength: "The length of phone should be 10 character long",
				number: "Please enter valid phone number",
			},					
        },
	});
  ...
  <div class="center_page_afterlogin">
    ...
    <div class="col_big">
        <div class="box_makepayment ">
            ...
            <?php echo form_open_multipart(site_url('topup-usa-pin-recharge/'.$results->ppnProductID), array('id'=>'topup_form', 'name'=>'topup_form'));?>
            ...
            <input type="text" name="recipientPhone" maxlength="10" value="<?php echo isset($recipientPhone)?$recipientPhone:''?>">
            <?php if (isset($error['recipientPhone'])) {?><span class="p155l red_color"><?php echo $error['recipientPhone'];?></span><div class="cb"></div><?php }?>
            <div class="cb"></div>
