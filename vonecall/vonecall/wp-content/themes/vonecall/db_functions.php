...
function getCustomerByPhone($phone)
{
    ...
    $customerDetails = $mydb->get_results("SELECT `c`.*, `s`.`stateName`, `at`.`alertTypeName`, `a`.`loginID` as agentLoginID 
...
function getCustomersLogin($where1) {
  $mydb = new wpdb(DB_USER, DB_PASSWORD, OLDER_DB, DB_HOST) or die('connection fail');
  $customerDetails = $mydb->get_results("SELECT * FROM tbl_Customer WHERE  ".$where1."");
     //print_r($mydb);
    return $customerDetails;
    
  }
function check_registered_email($where){

  $mydb = new wpdb(DB_USER, DB_PASSWORD, OLDER_DB, DB_HOST) or die('connection fail');
  $customerDetails = $mydb->get_results("SELECT * FROM tbl_Customer WHERE  ".$where."");
...
{
    $mydb = new wpdb(DB_USER, DB_PASSWORD, OLDER_DB, DB_HOST) or die('connection fail');
    $result = $mydb->insert('tbl_Customer', $data);
...
function getSettings($where='') {
  ...
  $mydb = new wpdb(DB_USER, DB_PASSWORD, OLDER_DB, DB_HOST) or die('connection fail');
  $settings = $mydb->get_results("SELECT * FROM tbl_GeneralSettings WHERE settingKey ='".$where."'");
...
function getSeqNo($agentID) {
  $mydb = new wpdb(DB_USER, DB_PASSWORD, OLDER_DB, DB_HOST) or die('connection fail');
   ...
   $agentDetails = $mydb->get_results("SELECT `p`.`seqNo` FROM (`tbl_CustomerPayment` as p) WHERE `p`.`customerID` = $agentID ORDER BY `p`.`seqNo` desc LIMIT 1 ");
...
function get_store_status($name){
  $mydb = new wpdb(DB_USER, DB_PASSWORD, OLDER_DB, DB_HOST) or die('connection fail');
   $agentDetails = $mydb->get_results("SELECT  log_status FROM `tbl_Agent` WHERE loginID = '".$name."' ");
  // print_r($mydb);
