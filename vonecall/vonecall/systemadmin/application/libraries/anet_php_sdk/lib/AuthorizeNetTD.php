...
class AuthorizeNetTD extends AuthorizeNetRequest
{
    ...
    public function getSettledBatchListForMonth($month = false, $year = false)
    {
        ...
        $firstSettlementDate = substr(date('c',time(0, 0, 0, $month, 1, $year)),0,-6);
        $lastSettlementDate  = substr(date('c',time(0, 0, 0, $month+1, 0, $year)),0,-6);
    ...
    public function getTransactionsForDay($month = false, $day = false, $year = false)
    {
        ...
        $day = ($day ? $day : date('d'));
        $year = ($year ? $year : date('Y'));
        $firstSettlementDate = substr(date('c',time(0, 0, 0, (int)$month, (int)$day, (int)$year)),0,-6);
        $lastSettlementDate  = substr(date('c',time(0, 0, 0, (int)$month, (int)$day, (int)$year)),0,-6);
