...
<div id="main">
  ...
  <div class="p20l p20r p13b">
    <div class="form_addcustomer p15t">
      ...
      <?php echo form_dropdown('distributor', $option_distributor, (isset($distributor)?$distributor:''), 'class="w327"');?>
      ...
      <input name="address2" value="<?php echo isset($address2)?$address2:'';?>" class="w325" name="" type="text" />
      ...
      <label>Login ID<span class="red_color">*</span></label>
...
<script>
function select_account_rep(value) {
	$.ajax({
		url: '<?php echo site_url('agent/get_max_store_commission');?>/'+value,
		type: 'POST',
		cache: false,
		data:{},
		dataType: "json",
		success: function(data) {
			for (i=1;i<=<?php echo $this->config->item('max_commission');?>;i++) {
				if (i<=data) {
					$("select[name=commissionRate] option[value='"+i+"']").show();
				} else {
					$("select[name=commissionRate] option[value='"+i+"']").hide();
				}
			}
		},
		error: function (){
